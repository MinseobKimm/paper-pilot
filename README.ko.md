...
| 단계 | 하는 일 | Paper Pilot artifact |
| --- | --- | --- |
| Collect | 로컬 PDF를 가져오고 폴더로 정리 | Library record |
| Encounter | 목차, 검색, 확대/축소, 링크 미리보기로 읽기 | Page text + metadata |
| Ask | 선택 텍스트, 페이지 맥락, 이미지 crop을 agent에게 전달 | AI result |
| Mark | 수동 또는 자동 하이라이트 저장 | Annotation |
| Interpret | 어려운 문장 번역, 요약, 설명 | Sidecar / panel result |
| Cite | 참고문헌 추출과 인용 이유 작성 | Citation card |
| Carry forward | 논문별 학습 상태 내보내기 | JSON or ZIP bundle |
...
Paper Pilot은 인터페이스를 특정 모델 provider 하나에 고정하지 않습니다. 앱은 구조화된 JSON 작업을 만들고, 선택된 agent가 처리한 뒤, 결과를 다시 로컬 작업 공간에 저장합니다.
...
sequenceDiagram
  ...
  participant Agent as Codex CLI 또는 Claude Code
  participant Inbox as bridge/inbox
  participant DB as SQLite

  UI->>App: 논문 맥락으로 task 생성
  App->>Outbox: task JSON 작성
  App->>Agent: worker process 시작
  Agent->>Inbox: result JSON 작성
  UI->>App: 결과 읽기
  App->>DB: 답변, 주석, 인용 저장
  DB-->>UI: 논문 작업 공간 갱신
```
